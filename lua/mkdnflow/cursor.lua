...
--   while continue do
--     local line = (reverse and vim.api.nvim_buf_get_lines(0, row - 1, row, false)) or
...
--       local has_heading = string.find(line[1], '^#+%s')
--       if has_heading then
...
local go_to_heading = function(anchor_text, reverse)
  ...
  local function isHeading(line_number)
    ...
    elseif not ts_success then
      -- Fallback to regex if Tree-sitter check fails or is not available
  ...
  while continue do
    ...
    if line then
      local has_heading = isHeading(row)
      if has_heading then
        if not anchor_text then
          vim.api.nvim_win_set_cursor(0, { row, 0 })
          continue = false
        else
          local heading_as_anchor = links.formatLink(line, nil, 2)
          if anchor_text == heading_as_anchor then
            vim.api.nvim_buf_set_mark(0, '`', position[1], position[2], {})
            vim.api.nvim_win_set_cursor(0, { row, 0 })
            continue = false
      ...
      if row == starting_row + 1 then
        continue = nil
        local message = anchor_text and ("⬇️  Couldn't find a heading matching " .. anchor_text .. "!") or
        "⬇️  Couldn't find a heading to go to!"
        if not silent then vim.api.nvim_echo({ { message, 'WarningMsg' } }, true, {}) end
      end
    else
      -- If the line does not have contents, start searching from the beginning
...
M.changeHeadingLevel = function(change)
  ...
  local is_heading = string.find(line[1], '^#+%s')
  if is_heading then
