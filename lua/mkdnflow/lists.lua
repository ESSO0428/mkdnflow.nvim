...
local get_siblings = function(row, indentation, li_type, up)
  ...
  while not done do
    local adj_line = (up and vim.api.nvim_buf_get_lines(0, row - 2, row - 1, false)[1]) or
...
update_parent_to_do = function(line, row, symbol)
  ...
  -- Check if the symbol represents an unchecked task
...
M.toggleToDo = function(row, status, meta)
  ...
  if meta then
    ...
    elseif string.lower(mode):match('v') then
      if not silent then
        vim.api.nvim_echo({ { '⬇️  Use simple visual mode (not line/block)', 'WarningMsg' } }, true,
          {})
      end
    else
      M.toggleToDo()
