...
local resolve_notebook_path = function(path, sub_home_var)
  ...
  local cur_file_dir_abs_path = vim.fn.expand('%:p:h')
...
local internal_open = function(path, anchor)
  ...
  if exists(path, 'd') and not exists(path_w_ext, 'f') then
    ...
    M.updateDirs()
    -- Inject the template
    if not path_w_ext:match("%.md$") then
      return
    end
...
local handle_external_file = function(path)
  ...
  elseif perspective.priority == 'root' and root_dir then
    ...
    escaped_path = this_os:match('Windows') and root_dir .. sep .. real_path or
        utils.escapeChars(root_dir .. sep .. real_path)
  elseif perspective.priority == 'first' or (perspective.priority == 'root' and perspective.fallback == 'first') then
    ...
    escaped_path = this_os:match('Windows') and initial_dir .. sep .. real_path or
        utils.escapeChars(initial_dir .. sep .. real_path)
  else
    ...
    local cur_file_dir = string.match(cur_file, '(.*)' .. sep .. '.-$')
    escaped_path = this_os:match('Windows') and cur_file_dir .. sep .. real_path or
        utils.escapeChars(cur_file_dir .. sep .. real_path)
  end
...
M.moveSource = function()
  ...
  local confirm_and_execute = function(derived_source, source, derived_goal, anchor, location, start_row, start_col,
                                       end_row, end_col)
    local truncated_goal = '...' .. truncate_path(derived_source, derived_goal)
    local prompt = "⬇️  Move '" ..
        derived_source .. "' (" .. source .. ") to '" .. truncated_goal .. "' (" .. location .. ")? [y/n] "
  ...
  local source, anchor, link_type, start_row, start_col, end_row, end_col = links.getLinkPart(links.getLinkUnderCursor(),
    'source')
        ...
        elseif source_exists then -- If the source location exists, proceed
          if dir then             -- If there's a directory in the goal location, ...
            ...
            if not to_dir_exists then
              ...
              confirm_and_execute(derived_source, source, derived_goal, anchor, location, start_row, start_col, end_row,
                end_col)
            end
          else -- Move
            confirm_and_execute(derived_source, source, derived_goal, anchor, location, start_row, start_col, end_row,
          ...
          vim.api.nvim_command("normal! :")
          vim.api.nvim_echo({ { '⬇️  ' .. derived_source .. ' doesn\'t seem to exist! Aborting.', 'WarningMsg' } },
            true, {})
        end
      end
    end)
