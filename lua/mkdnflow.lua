...
init.setup = function(user_config)
  ...
  init.initial_dir = (init.this_os:match('Windows') ~= nil and init.initial_buf:match('(.*)\\.-')) or
      init.initial_buf:match('(.*)/.-')
    ...
    if perspective.priority == 'root' then
      ...
      if root_tell then
        ...
        if init.root_dir then
          ...
          if not silent then
            vim.api.nvim_echo(
              { { '⬇️  No notebook found. Fallback perspective: ' .. fallback, 'WarningMsg' } }, true, {})
          end
